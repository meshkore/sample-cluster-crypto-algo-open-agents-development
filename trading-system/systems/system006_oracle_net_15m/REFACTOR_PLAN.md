...
3. **Modules are independent.** Each lives in `modules/<name>.py`, has a single
   ...
   (`python -m system006_oracle_net_15m.modules.<name> --explain`). Disable a module →
...
**Important:** `system002_intraday_momentum_5m`, `quantlab_ml`, `system004_llm_written_rules/05` and
`quantlab_core` are **not dead** — `orchestrator-manager/` (the Mac effort)
and the backtester/contract tests import them. Deleting them would break the
wider cluster. **Cleanup is scoped to `system006_oracle_net_15m` + `research/system06`**:
...
- **Phase 6 — cleanup & docs** ✅ Monolith removed, lookups centralised, `modules/
  README.md`, `python -m system006_oracle_net_15m.modules` introspection. (`system002_intraday_momentum_5m/
