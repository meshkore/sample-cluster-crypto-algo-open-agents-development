...
from quantlab_backtester.models import (
    ENGINE_VERSION,
    ExperimentSpec,
    ResearchState,
    utc_now,
...
-- Multi-run backtests (QUANT21). Everything below hangs off `backtest_id`, so
-- many runs coexist and each one's trades, orders, equity and statistics are
-- unambiguously its own. The older `portfolio_*` tables are keyed by
-- strategy_number and hold at most one run per strategy; they stay for the
-- autonomous loop, which is a single-champion pipeline, while anything a
-- contributor launches lands here.
