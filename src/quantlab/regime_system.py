...
tactic: the 20-bar breakout is the strongest cell in both SIDEWAYS and BULL.
What regime separates is *magnitude* -- every tactic earns less in BEAR. So the
regime's honest job in this system is exposure governance, expressed through
`*_weight`, and the branches differ mainly in how much confirmation they demand
...
BRANCHES: dict[MarketRegime, type] = {
    MarketRegime.BULL: _BullTrendBranch,
    MarketRegime.SIDEWAYS: _SidewaysBreakoutBranch,
    MarketRegime.BEAR: _BearParticipationBranch,
}
...
class _RegimeRouter:
    ...
    **A regime change forces flat for one bar.** When the label moves, the
    ...
    def __init__(self, params: dict[str, Any], context: MarketContext | None = None):
        ...
        self.branches = {
            regime: builder(params) for regime, builder in BRANCHES.items()
        }
        self.reset()
