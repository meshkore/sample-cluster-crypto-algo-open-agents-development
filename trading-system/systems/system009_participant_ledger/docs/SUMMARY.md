...
> | | | |
> |---|---|---|
> | **Phase 1** | reconstruct the whole record, every asset from the day it first trades, liquidity and players managed through time, balances set at 2025-12-31 | **BUILT** - `python -m system009_participant_ledger.phase1` |
> | **Phase 2** | train the model on how the players actually behaved | **BUILT** - `python -m system009_participant_ledger.train` |
> | **Phase 3** | forward-test from 2026-01-01, simulate trading as close to reality as possible, measure the success ratio | **RUN** - `python -m system009_participant_ledger.phase3`, and see the caveat below |
...
$4T. Nothing in v1 ever tested it. `python -m system009_participant_ledger.calibrate` now does, per asset,
