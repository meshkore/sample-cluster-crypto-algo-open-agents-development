...
A second trading system beside System Four (`quantlab_core/`), which it does
...
python3 -m system002_intraday_momentum_5m.prepare

# every iteration after that: 12 training blocks + the sealed 2026 window
python3 -m system002_intraday_momentum_5m.launch --phase both
python3 -m system002_intraday_momentum_5m.launch --phase both --set stop_atr=2.5 --set target_atr=1.5
```
...
```python
# trading-system/system002_intraday_momentum_5m/mynewidea.py
...
```bash
python3 -m system002_intraday_momentum_5m.launch --brain intraday-breakout --phase both
```
...
`intraday-reversion` is in `brains.available()` as soon as `system002_intraday_momentum_5m`
