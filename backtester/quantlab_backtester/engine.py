...
class SizingPolicy(Protocol):
    """What the engine needs from a money-management policy, and nothing more.

    Structural, not nominal: the backtester never imports the trading system, so
    any object carrying these members can be scored. That is the seam that keeps
    the instrument immutable while the decisions above it stay open to any
    paradigm a contributor wants to argue for.

    Deliberately expressed as attribute reads rather than a base class -- a
    contributor supplying a policy should not have to inherit from the thing
    that grades them.
    """

    risk_per_trade: float
    maximum_position_fraction: float
    minimum_position_fraction: float
    stop_loss_pct: float
    take_profit_pct: float
    minimum_confidence: float
    maximum_concurrent_assets: int
    ...
    drawdown_deleverage_start: float
...
class LongOnlyPortfolioBacktester:
    ...
    def __init__(self, costs: CostModel, policy: SizingPolicy):
