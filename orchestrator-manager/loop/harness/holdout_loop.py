...
def run(label, overrides, start=HOLDOUT):
    ...
    # Tell the loop this iteration is alive. A six-cell sweep over 386 assets
    # outlasted the old 90-minute stale window and got its own live iteration
    # reclaimed as ABANDONED.
    subprocess.run(
        [
            sys.executable,
            str(Path(__file__).parent.parent / "bin" / "ledger.py"),
            "heartbeat",
        ],
        capture_output=True,
    )
...
if __name__ == "__main__":
    ...
    print(f"HOLDOUT 2022-01-01 -> 2025-12-31, {len(BARS)} assets\n", flush=True)
