...
- #QUANT17 in progress — the drawdown mandate. The operator spotted a flat line on the equity chart from mid-2021 and it was not a cash position: `max_drawdown` was measured from the running PEAK and drove the de-leverage ramp, so once equity sat ~24.7% below the peak the risk budget collapsed, nothing could open, equity could not grow, and therefore the peak never updated and the drawdown never shrank. **A one-way ratchet — S00852 last opened a position on 2021-05-19 and held nothing for four and a half years while reporting itself legal.** `drawdown_basis` is now explicit ("peak" default, so nothing stored moves; config set to "initial"). Same strategy, only the basis changed: **+1480% -> +2836.21%, 941 -> 2,232 trades, last active 2021-05-18 -> 2025-12-31, and the account never fell more than 2.96% below the 100,000 deposit either way.** Walk-forward went from not-eligible to eligible (7/12). One continuous account 2017->today: **100,000 becomes $2.81M**, still trading, capital drawdown 2.96% — while the control is ABORTED under both mandates at -28.37%. Also fixed three gates that hardcoded the peak rule and were silently discarding the result (forward eligibility, the dashboard high-water query and its ranking), and made the equity chart stop where trading stopped with a red exit marker.
- #QUANT16 in progress — bear-regime participation and the 2026 forward test. The detector called **BEAR on 2025-12-21**, eleven days before the forward window, and held it through 100% of 2026 while the composite fell 40.6% (BTC -28.2%, median asset -47.2%, only 40 of 399 positive). Cross-sectional momentum inside bear regimes is **rejected** — every decile negative, spread -0.13%. What works is *absolute* strength: above own 200d AND 50d returns +3.13% forward 30d against the -5.41% bear baseline. Rebuilt the bear branch on that and moved the router to the wide daily universe. **S00852: Phase-1 +1480.02% at 24.72% DD (legal) against the control's +23.77% which ABORTED at 30.01% — +1456 points at 5.3 points less drawdown, the lab's best Phase-1 result by an order of magnitude; walk-forward 7/12 but NOT eligible (drawdown breach in 1 fold); forward 2026 -0.80% at 6.60% DD.** Goal of beating the +4.33% incumbent **not met** — though that incumbent is the max of 313 forward runs with a -1.47% median and 14 positive, i.e. almost certainly a multiple-testing artifact. Also fixed a policy-reconstruction bug: hand-maintained key lists in both evaluators had dropped `drawdown_deleverage_end` (and `minimum_position_fraction` in forward.py), silently widening the de-leverage ramp; now derived from the dataclass with a regression test.
